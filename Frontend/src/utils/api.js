...
export const api = {
  ...
  // Retrieval
  searchItem: (params) =>
    fetch(`${API_BASE}/search?${new URLSearchParams(params)}`).then((res) =>
      res.json()
    ),
  ...
  // Waste Management
  identifyWaste: () =>
    fetch(`${API_BASE}/waste/identify`).then((res) => res.json()),
  ...
  exportArrangement: () =>
    fetch(`${API_BASE}/export/arrangement`).then((res) => res.blob()),

  // Logs
  getLogs: (params) =>
    fetch(`${API_BASE}/logs?${new URLSearchParams(params)}`).then((res) =>
      res.json()
    ),
};
